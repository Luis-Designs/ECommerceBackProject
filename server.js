    ...
    if (req.method === 'POST') {
      ...
      if (req.path === '/session') {
        const isUser = await User.findOne({
          attributes: ['firstName', 'lastName', 'email', 'token', 'role'],
        ...
        if (isEmail || isId) {
          res.send({
            status: '409',
            data: isEmail ? 'Email already exist' : 'Id already exist',
          });
        } else {
          next();
        }
        ...
        if (Array.isArray(isProduct) && isProduct.length > 0) {
          ...
          res.send({
            status: '400',
            data: `Your search - '${req.body.productName}' - didn't match any product.`,
          });
        }
        ...
        if (isCategory) {
          res.send({
            status: '400',
            data: `Category '${req.body.newCategory}' already exist`,
          });
        } else {
          next();
        }
        ...
        const foundOpenCart = isShoppingCart.find(
          ({ id: id1 }) =>
            !isOrder.some(({ shoppingCartId: id2 }) => id1 === id2)
        );
      ...
      if (req.path === '/category') {
        ...
        if (isCategory) {
          res.send({
            status: '409',
            data: `Category '${newCategoryName}' already exist`,
          });
        } else {
          next();
        }
        ...
        const isProductCart = await ProductCart.findOne({
          where: {
            productId: productId,
      ...
      if (req.path === '/productCart') {
        ...
        const isProductCart = await ProductCart.findAll({
          where: {
            shoppingCartId: shoppingCartId,
          },
        });
...
app.post('/user', async (req, res) => {
  // Sign up request
  const { id, email, password, fname, lname, city, street, role, token } =
    req.body;
  ...
  try {
    ...
    if (isProduct) {
      ...
      const isCategory = categoryId
        ? await Category.findOne({ where: { name: categoryId } })
        : null;
      const uploadResponse = img
        ? await cloudinary.uploader.upload(img, {
            upload_preset: 'shoppingProject',
          })
        : null;
...
app.delete('/productCart', (req, res) => {
